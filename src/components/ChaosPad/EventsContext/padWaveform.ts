...
export type PadWaveformStore = {
	getBins: () => Float32Array
	getVersion: () => number
	apply: (prev: { nx: number; ny: number } | null, nx: number, ny: number) => void
	reset: () => void
	subscribe: (cb: () => void) => () => void
}

export function createPadWaveformStore(): PadWaveformStore {
	const bins = new Float32Array(PAD_WAVEFORM_BINS)
	let version = 0
	...
	return {
		getBins: () => bins,
		getVersion: () => version,
		apply(prev: { nx: number; ny: number } | null, nx: number, ny: number) {
			applySegment(bins, prev?.nx ?? nx, prev?.ny ?? ny, nx, ny)
			version++
			notify()
		},
		reset() {
